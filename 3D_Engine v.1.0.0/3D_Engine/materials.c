#include "materials.h"
#include "textures.h"  // Assuming textures.h has necessary texture loading functions
#include <stdio.h>
#include <string.h>
...
void loadPBRTextures() {
    ...
    PBRMaterial peacockOre = loadPBRMaterial(
        ...
        "resources/materials/peacock-ore-unity/peacock-ore_height.png",  // Note: Height as roughness is a placeholder
    ...
    PBRMaterial chunkyRockface = loadPBRMaterial(
        ...
        "resources/materials/stylized-chunky-rockface-unity/stylized-chunky-rockface_height.png",  // Note: Height as roughness is a placeholder
    ...
    PBRMaterial stainlessSteel = loadPBRMaterial(
        ...
        "resources/materials/used-stainless-steel2-unity/used-stainless-steel2_height.png",  // Note: Height as roughness is a placeholder
...
void addMaterial(const char* name, PBRMaterial material) {
    ...
    materialNames[materialCount] = _strdup(name);  // Ensure unique names for each material
...
PBRMaterial* getMaterial(const char* name) {
    ...
    // Use peacockOre as the default material
    for (int i = 0; i < materialCount; i++) {
        if (strcmp(materialNames[i], "peacockOre") == 0) {
            return &materials[i];
        }
    }
    return NULL; // This should not happen if peacockOre is properly loaded
}
