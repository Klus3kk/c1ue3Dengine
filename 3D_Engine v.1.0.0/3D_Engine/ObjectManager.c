...
void addObject(Camera* camera, ObjectType type, bool useTexture, int textureIndex, bool colorCreation, Model* model, PBRMaterial material, bool usePBR) {
    ...
    if (textureIndex == -1) {
        newObject.object.textureID = getTexture("bricks"); // Assuming "bricks" is the default texture
    }
...
void removeObject(int index) {
    ...
    // Clear the last object (optional but good practice)
